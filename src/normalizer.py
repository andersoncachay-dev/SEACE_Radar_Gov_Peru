...
COLUMN_MAP = {
    ...
    "Descripción de Requerimiento": "descripcion",
    "Descripcion de Requerimiento": "descripcion",
    "N° de Requerimiento": "nomenclatura",
    ...
    "VR / VE / Cuantía de la contratacion": "monto",
    "Monto": "monto",
    "Moneda": "moneda",
    "Versión SEACE": "version_seace",
    ...
    # OCDS flattened columns
    "ocid": "nomenclatura",
    "main_ocid": "nomenclatura",
    "id": "nomenclatura",
    "main_id": "nomenclatura",
    "buyer.name": "entidad",
    "tender.procuringEntity.name": "entidad",
    "tender.mainProcurementCategory": "objeto",
    "tender.title": "descripcion",
    "tender.description": "descripcion",
    "tender.value.amount": "monto",
    "value.amount": "monto",
    "contracts.value.amount": "monto",
    "date": "fecha_publicacion",
    "tender.tenderPeriod.startDate": "fecha_publicacion",
    "tender.tenderPeriod.endDate": "fecha_presentacion",
...
def normalize_columns(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for col in set(COLUMN_MAP.values()):
        if list(df.columns).count(col) > 1:
            df = _coalesce_duplicate_columns(df, col)
    for col in REQUIRED:
        if col not in df.columns:
            df[col] = None

    # If descripcion empty, build from any descriptive text columns
    text_candidates = [c for c in df.columns if any(k in c.lower() for k in ["description", "title", "objeto", "item", "name"])]
    if text_candidates:
