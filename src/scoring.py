...
def calcular_score(row):
    descripcion=f"{row.get('descripcion','')} {row.get('objeto','')} {row.get('nomenclatura','')}".lower(); entidad=str(row.get('entidad','')).lower(); region=str(row.get('region','')).lower(); monto=row.get('monto',0); estado=str(row.get('estado_comercial','')).lower(); score=0; motivos=[]
    ...
    if 'consultas y propuesta' in estado: score+=20; motivos.append('Vigente para consultas y propuesta')
    elif 'solo para propuesta' in estado or 'sólo para propuesta' in estado: score+=15; motivos.append('Vigente solo para propuesta')
    elif 'cerrado' in estado: score=max(0,score-30); motivos.append('Proceso cerrado')
