...
def detectar_sector(row):
    ...
    if "ugel" in texto or "educativa" in texto or "colegio" in texto or "institucion educativa" in texto: return "Educacion"
    if "diresa" in texto or "red de salud" in texto or "salud" in texto or "hospital" in texto: return "Salud"
...
def calcular_score(row):
    ...
    entidad = str(row.get("entidad", "")).lower(); region = str(row.get("region", "")).lower(); monto = row.get("monto", 0); dias = row.get("dias_presentacion", None)
    score = 0; motivos = []
...
def enriquecer_oportunidades(df):
    df = df.copy(); scores = df.apply(calcular_score, axis=1, result_type="expand")
