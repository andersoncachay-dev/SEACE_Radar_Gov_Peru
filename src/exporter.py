...
def build_excel(oportunidades: pd.DataFrame) -> bytes:
    ...
    resumen = pd.DataFrame({
        "indicador": ["total_oportunidades", "prioridad_A", "prioridad_B", "prioridad_C", "monto_total"],
        "valor": [
            len(df),
            int((df.get("prioridad") == "A").sum()) if "prioridad" in df else 0,
            int((df.get("prioridad") == "B").sum()) if "prioridad" in df else 0,
            int((df.get("prioridad") == "C").sum()) if "prioridad" in df else 0,
            float(df.get("monto", pd.Series(dtype=float)).sum()) if "monto" in df else 0,
        ],
    })
